...
class MetadataLoader(BaseSQliteDB):
    ...
    def build_diff(self):
        ...
        for t in targets:
            ...
            tgt_json = from_b64(raw[0]) if self.ub64 else raw[0]
            c_json = from_b64(c_json_raw) if self.ub64 else c_json_raw
            ...
            diff_json_out = to_b64(reg_json_diff) if self.ub64 else reg_json_diff.replace("'", "''")
            tgt_json_out = to_b64(tgt_json) if self.ub64 else tgt_json.replace("'", "''")
